 ...
 */
@NamedQueries(value={
	@NamedQuery(name="bugsSummary",query="find (name, email) join loggedBugs (title, status) where id=:id  "),
	@NamedQuery(name="bugStatus",query="join loggedBugs where loggedBugs.status.code = :bugStatus order by name")
})
@Entity
@Table(name="s_user")
public class User implements Serializable {

	private static final long serialVersionUID = -5892617285254044274L;
