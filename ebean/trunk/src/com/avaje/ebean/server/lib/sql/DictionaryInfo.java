...
public class DictionaryInfo implements Serializable {
	...
	private boolean isRegisteredSearchPattern(TableSearch tableSearch) {
		...
		synchronized (monitor) {
			...
			// return true if the pattern was already registered
	...
	private int searchForTables(TableSearch tableSearch) {
		...
		synchronized (monitor) {
			...
			if (dictionaryCase == CaseUnknown) {
				// search using the tableName as it is passed in... no case
				// conversion...
			...
			// no tables found to match the patterns
			return 0;
		}
	}
	...
	private int searchForTablesDetail(TableSearch tableSearch) {
		...
		synchronized (monitor) {
			try {
				...
				// register the pattern... only perform a pattern once
	...
	public static String stripQuotesLowerCase(String tableName) {
		...
		if (!Character.isLetterOrDigit(firstChar)) {
			if (firstChar != '_') {
				// assumed to have quoted identifiers
	...
	public TableInfo getTableInfo(String fullTableName) {
		...
		synchronized (monitor) {
			...
			if (tableInfo == null) {
				// try to load it... using as is, lowerCase, upperCase...
	...
	public IntersectionInfo findIntersection(String sourceTable, String destTable) throws SQLException {
		...
		synchronized (monitor) {
			...
			// search all the source exported keys
			...
			for (int i = 0; i < srcExpKeys.length; i++) {
				...
				// is this the intersection table?
	...
	private TableInfo searchForOneTable(TableSearchName searchName, String unquotedLowerCaseTableName) {
		...
		synchronized (monitor) {
			...
			try {
				...
				if (usingQuotes) {
					// search using quoted identifiers
					...
					if (tableInfo != null) {
						// found it using quoted identifiers
						return tableInfo;
					}
				}

				// search removing quotes if they existed
				tableSearch.setSchemaPattern(searchName.getSchema());
				...
				if (dictionaryCase != CaseLower) {
					// search using upper case which is 'standard' for unquoted
					// identifiers
				...
				if (dictionaryCase != CaseUpper) {
					// search using lower case which is used by 'postgres'
