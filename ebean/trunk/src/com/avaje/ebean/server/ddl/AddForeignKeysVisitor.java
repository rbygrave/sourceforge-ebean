...
public class AddForeignKeysVisitor implements BeanVisitor {
	...
	public static class FkeyPropertyVisitor extends BaseTablePropertyVisitor {
		...
		public void visitOneImported(BeanPropertyAssocOne<?> p) {
			...
			String fkName = ctx.getNamingConvention().getForeignKeyName(p, ctx.incrementFkCount());
			...
			if (ctx.getDdlSyntax().isRenderIndexForFkey()){
				...
				String idxName = ctx.getNamingConvention().getIndexName(p, ctx.incrementIxCount());
				if (idxName != null){
					ctx.write(idxName);
				}
