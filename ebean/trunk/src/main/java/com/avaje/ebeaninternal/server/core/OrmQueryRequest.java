...
public final class OrmQueryRequest<T> extends BeanRequest implements BeanQueryRequest<T>, SpiOrmQueryRequest<T> {
     ...
     * transaction is rolled back by the background fetching thread. Background
     * fetching always takes place in its own transaction.
     */
    ...
    /**
     * Set an Query object that owns the PreparedStatement that can be
     * cancelled.
     */
