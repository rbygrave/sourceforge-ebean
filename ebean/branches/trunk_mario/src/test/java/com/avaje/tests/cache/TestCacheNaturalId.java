...
public class TestCacheNaturalId extends TestCase {
	...
	public void test(){
		...
		List<Contact> list = Ebean.find(Contact.class)
		...
		Contact c0 = Ebean.find(Contact.class)
			.where().eq("email", emailToSearch)
			.findUnique();
		
		ServerCacheStatistics stats0 = contactCache.getStatistics(false);
		
		Contact c1 = Ebean.find(Contact.class)
			.where().eq("email", emailToSearch)
			.findUnique();
		...
		Contact c2 = Ebean.find(Contact.class)
			.where().eq("email", "mychangedemail@what.com")
			.findUnique();
