...
import org.avaje.ebean.server.core.BootupClassPathSearch;
...
public class PluginFactory {
	...
	public PluginFactory(ConfigProperties baseProperties,
			ClusterManager clusterManager) {
		this.clusterManager = clusterManager;
		this.bootupClassSearch = new BootupClassPathSearch(null, baseProperties);

		boolean search = baseProperties.getBooleanProperty(
				"ebean.classpath.search", true);
		if (search) {
	 ...
	 */
	public Plugin create(String name, DataSource ds,
			ServerConfiguration serverConfig, ConfigProperties props) {
		...
		PluginProperties properties = createProperties(name, ds, props,
				bootupClasses);
	...
	private PluginProperties createProperties(String name, DataSource ds,
			ConfigProperties props, BootupClasses bootupClasses) {
	...
	private Plugin createPlugin(PluginCore pluginCore) {
		...
		try {
			...
			if (classname != null) {
				...
				return (Plugin) FactoryHelper.create(classname, CONS_TYPES,
						args);
